...
class PostgresStorage:
    def __init__(self, session: AsyncSession, station_data: list[StationData]) -> None:
    ...
    async def execute(self) -> None:
        try:
            for station_data in self._station_data:
                ...
                parameter_type_id = self._get_parameter_type(station_data.parameter_type)
                if not station_id or not parameter_type_id:
                    return
                parameter_id = self._get_parameter(pt_id=parameter_type_id, station_id=station_id)
                if not parameter_id:
                    return
                await self._create_measure(self._build_measure(station_data, parameter_id))
    ...
    async def get_active_detect_types(session):
        query = select(ParameterType.detect_type).where(ParameterType.is_active == True)
        result = await session.execute(query)
        return [row[0] for row in result.all()]

    async def _get_parameter_type(self, parameter_types: list[ParameterType]) -> None | ParameterTypeId:
    ...
    async def _get_station(self, uid: int) -> None | StationId:
        query = select(WeatherStation.id).where(
            WeatherStation.uid == uid,
            WeatherStation.is_active == True
        )
        result = await self._session.execute(query)
        return result.scalar()

    async def _get_parameter(self, pt_id: int, station_id: int) -> None | ParameterId:
    ...
    def _build_measure(self, station_data: StationData, parameter_id: int) -> MeasureData:
